...
def build_message():
    ...
    return command.pack(master.mav)[6:-2]
...
#packed_message = command_message.pack()
#payload = packed_message[6:-2]  # only payload
...
header.extend(bytearray([1])) # inc
header.extend(bytearray([0])) # cmp
header.extend(bytearray([0]))  # Sequence number
