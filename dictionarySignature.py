...
def calculate_secretkey_input(hashed_seed, header, payload, crc, linkid, timestamp):
    signaturecompleta = hashed_seed + header.hex() + payload.hex() + crc.hex() + linkid.to_bytes(1,
                                                                                                 'big').hex() + timestamp.hex()
        ...
        with open(wordlist, 'r') as wl:
            ...
            for guess in guesses:
                ...
                if hashed_guess == hash_string:
                    ...
                    if args.v:
                        print(bcolors.FAIL + "Fail \"" + guess + "\"" + bcolors.ENDC + " (" + str(
                            guesses.index(guess) + 1) + "/" + str(len(guesses)) + ")")
...
parser.add_argument('--v', help="(true/false) Show more information while cracking", default=False,
